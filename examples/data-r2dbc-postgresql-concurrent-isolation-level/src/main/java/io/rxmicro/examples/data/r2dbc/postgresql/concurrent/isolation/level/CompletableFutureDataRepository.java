...
public interface CompletableFutureDataRepository {

    CompletableFuture<Transaction> begin(IsolationLevel isolationLevel);

    @Select("SELECT * FROM ${table} WHERE id=?")
    CompletableFuture<Account> findAccount(Transaction transaction, long id);
}
