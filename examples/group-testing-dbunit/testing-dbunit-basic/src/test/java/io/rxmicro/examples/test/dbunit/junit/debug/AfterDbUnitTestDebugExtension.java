...
public class AfterDbUnitTestDebugExtension extends BaseDebugExtension implements
        BeforeAllCallback, BeforeTestExecutionCallback,
        AfterTestExecutionCallback, AfterAllCallback {

    @Override
    public void beforeAll(final ExtensionContext context) {
        log(context, "<after:before>  ALL");
    }
    ...
    public void beforeTestExecution(final ExtensionContext context) {
        ...
        log(
                context,
                "<after:before>  [?] Docker container socket: jdbc:postgresql://?:?/rxmicro",
                method.getName(), container.getContainerIpAddress(), container.getFirstMappedPort()
        );
        log(
                context,
                "<after:before>  [?] TestDatabaseConfig.jdbc: ?",
    ...
    public void afterTestExecution(final ExtensionContext context)  {
        ...
        log(
                context,
                "<after:after>   [?] Docker container socket: jdbc:postgresql://?:?/rxmicro",
                method.getName(), container.getContainerIpAddress(), container.getFirstMappedPort()
        );
        log(
                context,
                "<after:after>   [?] TestDatabaseConfig.jdbc: ?",
                method.getName(), config.getJdbcUrl()
        );
    }
