...
package io.rxmicro.benchmark.reactive;

import io.rxmicro.benchmark.reactive.lib.ReactiveLibAsyncBenchmark;
...
public final class Launcher {
    ...
    public static void launchBenchmark(final Class<?> benchmarkClass) throws RunnerException {
        final Options opts = new OptionsBuilder().include(benchmarkClass.getName())
                ...
                .result(format("%sreactive-%s.txt", RESULT_FOLDER, benchmarkClass.getSimpleName()))
                .build();
        new Runner(opts).run();
    }

    public static void main(final String[] args) throws RunnerException {
        //launchBenchmark(ReactiveLibSyncBenchmark.class);
        launchBenchmark(ReactiveLibAsyncBenchmark.class);
    }
}
