В данном разделе будут подробно описаны шаги, которые необходимо выполнить, чтобы с помощью фреймворка *RxMicro* создать REST-based микросервис возвращающий сообщение `"Hello World!"`.
...
[NOTE]
====
Возможностей версии *IntelliJ IDEA Community Edition* достаточно для полноценной и удобной работы над проектом, использующим фреймворк RxMicro.
====

RxMicro фреймворк состоит из нескольких десятков модулей, поэтому для удобства управления ими рекомендуется, чтобы на Вашем компьютере был также установлен https://maven.apache.org/[`maven`^].
