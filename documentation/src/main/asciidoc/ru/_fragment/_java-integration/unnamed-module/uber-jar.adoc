...
===== Enable `unnamed module` mode

To enable `unnamed module` mode, add the `RX_MICRO_BUILD_UNNAMED_MODULE` compiler argument to `maven-compiler-plugin`:
...
<1> `RxMicro Annotation Processor` will use `unnamed module` mode for current microservice project.
...
_(Подробнее об виртуальном дескрипторе можно ознакомится в разделе <<java-integration-unnamed-module-module-config>>.)_
...
`uber.jar` содержит исходный код микросервисного проекта, код фреймворков RxMicro и Netty:
...
[NOTE]
====
To run a modularized micro service project java requires more arguments:
