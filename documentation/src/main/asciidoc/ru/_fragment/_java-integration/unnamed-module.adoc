...
Для интеграции с библиотеками, которые не поддерживают JPMS, the RxMicro framework позволяет активировать режим `unnamed module` для `RxMicro Annotation Processor`.
...
<1> `RxMicro Annotation Processor` will use the `unnamed module` mode for the current microservice project.
