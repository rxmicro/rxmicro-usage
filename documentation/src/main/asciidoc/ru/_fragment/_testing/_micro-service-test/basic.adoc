...
Процесс тестирования REST-based микросервиса заключается в том, чтобы после того как RxMicro запустит тестируемый микросервис на HTTP сервере выполнить HTTP запрос.
...
<2> Для выполнения *блокирующих* HTTP запросов, RxMicro поддерживает специальный компонент:
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-test/src/main/java/io/rxmicro/test/BlockingHttpClient.java[`BlockingHttpClient`^].
Разработчик должен объявить ссылку на данный компонент и в процессе запуска теста, фреймворк RxMicro автоматически внедрит созданный объект класса
