...
Такой результат связан с тем, что создав альтернативу-мок для компонента `DataRepository`, в процессе тестирования не используются сгенерированные фреймворком RxMicro классы для работы postgres репозитория.
