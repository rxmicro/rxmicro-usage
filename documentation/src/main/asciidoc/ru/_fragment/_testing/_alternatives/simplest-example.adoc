...
----
<1> Альтернативы поддерживаются фреймворком RxMicro только при тестировании компонентов или REST-based микросервисов.
Поэтому необходимо объявить, что данный тест является тестом компонента `ParentComponent`.
<2> Объект тестируемого компонента будет создан фреймворком RxMicro автоматически.
