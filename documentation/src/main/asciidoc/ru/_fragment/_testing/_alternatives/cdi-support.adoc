...
Если разработчик в проекте использует модуль <<{cdi}#cdi-section,`rxmicro.cdi`>>, то тогда *все* пользовательские компоненты считаются бинами и подчиняются алгоритму внедрения альтернатив RxMicro компонентов.
...
[TIP]
====
Приведенный выше пример является демонстрацией возможностей альтернатив и CDI контейнера.
