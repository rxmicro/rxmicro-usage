...
. Дополнительную аннотацию {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-test-junit/src/main/java/io/rxmicro/test/junit/RxMicroIntegrationTest.java[`@RxMicroIntegrationTest`^], которая информирует RxMicro о необходимости создать HTTP клиент и внедрить его в тестируемый класс.
