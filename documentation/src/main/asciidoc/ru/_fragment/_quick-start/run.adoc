=== Запуск микросервиса

==== Запуск используя IDE:

Запустить REST-based микросервис можно используя контекстное меню запуска IntelliJ IDEA

.Run the REST-based micro service using IntelliJ IDEA context menu.
image::quick-start/run-using-ide.jpg[]

[TIP]
====
Если в процессе запуска REST-based микросервиса `HelloWorldMicroService` Вы получили следующую ошибку:

image::quick-start/kotlin-error-fix.jpg[]

пересоберите проект!

_(Для этого выполните команду *Rebuild project* из главного меню: `Build` -> `Rebuild project`.)_
====

После запуска в консоли отобразится следующая информация:

[source,text]
----
2020-02-02 20:14:11.707 [INFO] io.rxmicro.rest.server.netty.internal.component.NettyServer :
Server started at 0.0.0.0:8080 using NETTY transport in 500 millis # <1>
----
<1> Сообщение формата `Server started in ... millis` означает, что RxMicro HTTP сервер успешно запустился.

[WARNING]
====
Если произошла ошибка в процессе запуска, в консоли будет отображен stack trace возникшей ошибки.
====

==== Запуск с помощью терминала:

Перейдите в папку `target` микросервисного проекта, запустите терминал в этой папке и выполните следующую команду:

[source,text]
----
java --add-opens=java.base/jdk.internal.misc=io.netty.common -Dfile.encoding=UTF-8 -p ./classes:$M2_REPO/io/rxmicro/rxmicro-rest-server-netty/0.1/rxmicro-rest-server-netty-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-rest-server/0.1/rxmicro-rest-server-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-rest/0.1/rxmicro-rest-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-model/0.1/rxmicro-model-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-http/0.1/rxmicro-http-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-logger/0.1/rxmicro-logger-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-common/0.1/rxmicro-common-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-config/0.1/rxmicro-config-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-runtime/0.1/rxmicro-runtime-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-files/0.1/rxmicro-files-0.1.jar:$M2_REPO/io/netty/netty-codec-http/4.1.44.Final/netty-codec-http-4.1.44.Final.jar:$M2_REPO/io/netty/netty-common/4.1.44.Final/netty-common-4.1.44.Final.jar:$M2_REPO/io/netty/netty-buffer/4.1.44.Final/netty-buffer-4.1.44.Final.jar:$M2_REPO/io/netty/netty-codec/4.1.44.Final/netty-codec-4.1.44.Final.jar:$M2_REPO/io/netty/netty-handler/4.1.44.Final/netty-handler-4.1.44.Final.jar:$M2_REPO/io/netty/netty-transport/4.1.44.Final/netty-transport-4.1.44.Final.jar:$M2_REPO/io/netty/netty-resolver/4.1.44.Final/netty-resolver-4.1.44.Final.jar:$M2_REPO/io/rxmicro/rxmicro-rest-server-exchange-json/0.1/rxmicro-rest-server-exchange-json-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-exchange-json/0.1/rxmicro-exchange-json-0.1.jar:$M2_REPO/io/rxmicro/rxmicro-json/0.1/rxmicro-json-0.1.jar -m examples.quick.start/io.rxmicro.examples.quick.start.HelloWorldMicroService
----

[TIP]
====
Подразумевается, что на Вашем компьютере установлена переменная окружения `M2_REPO`.
Данная переменная должна указывать на локальный репозиторий maven.

* Для Linux систем: `M2_REPO` = `/home/$USER/.m2/repository`.
* Для Osx систем: `M2_REPO` = `/Users/$USER/.m2/repository`.
* Для Windows систем: `M2_REPO` = `C:\Documents and Settings\%USERNAME%\.m2\repository`.
====

include::__note_run_on_windows.adoc[]

После запуска в консоли отобразится следующая информация:

[source,text]
----
java --add-opens=java.base/jdk.internal.misc=io.netty.common
-p ./classes:
$M2_REPO/io/rxmicro/rxmicro-rest-server-netty/0.1/rxmicro-rest-server-netty-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-rest-server/0.1/rxmicro-rest-server-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-rest/0.1/rxmicro-rest-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-model/0.1/rxmicro-model-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-http/0.1/rxmicro-http-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-logger/0.1/rxmicro-logger-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-common/0.1/rxmicro-common-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-config/0.1/rxmicro-config-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-runtime/0.1/rxmicro-runtime-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-files/0.1/rxmicro-files-0.1.jar:
$M2_REPO/io/netty/netty-codec-http/4.1.44.Final/netty-codec-http-4.1.44.Final.jar:
$M2_REPO/io/netty/netty-common/4.1.44.Final/netty-common-4.1.44.Final.jar:
$M2_REPO/io/netty/netty-buffer/4.1.44.Final/netty-buffer-4.1.44.Final.jar:
$M2_REPO/io/netty/netty-codec/4.1.44.Final/netty-codec-4.1.44.Final.jar:
$M2_REPO/io/netty/netty-handler/4.1.44.Final/netty-handler-4.1.44.Final.jar:
$M2_REPO/io/netty/netty-transport/4.1.44.Final/netty-transport-4.1.44.Final.jar:
$M2_REPO/io/netty/netty-resolver/4.1.44.Final/netty-resolver-4.1.44.Final.jar:
$M2_REPO/io/rxmicro/rxmicro-rest-server-exchange-json/0.1/rxmicro-rest-server-exchange-json-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-exchange-json/0.1/rxmicro-exchange-json-0.1.jar:
$M2_REPO/io/rxmicro/rxmicro-json/0.1/rxmicro-json-0.1.jar
-m examples.quick.start/io.rxmicro.examples.quick.start.HelloWorldMicroService

2020-02-02 20:14:11.707 [INFO] io.rxmicro.rest.server.netty.internal.component.NettyServer :
Server started at 0.0.0.0:8080 using NETTY transport in 500 millis # <1>
----
<1> Сообщение формата `Server started in ... millis` означает, что RxMicro HTTP сервер успешно запустился.

В процессе запуска микросеривса с помощью терминала очень неудобно перечислять все зависимости и их версии.
Для решения данной проблемы можно использовать `maven-dependency-plugin`, который умеет копировать все зависимости проекта.
Для активации `maven-dependency-plugin`, необходимо добавить его в `pom.xml`:

[source,xml]
----
<plugin>
    <artifactId>maven-dependency-plugin</artifactId>
    <version>${maven.dependencies.plugin.version}</version><!--1-->
    <executions>
        <execution>
            <id>copy-dependencies</id>
            <phase>package</phase><!--2-->
            <goals>
                <goal>copy-dependencies</goal>
            </goals>
            <configuration>
                <outputDirectory>${project.build.directory}/lib</outputDirectory><!--3-->
                <includeScope>compile</includeScope><!--4-->
            </configuration>
        </execution>
    </executions>
</plugin>
----
<1> {MAVEN-REPO-SEARCH-ROOT}org.apache.maven.plugins/maven-dependency-plugin[Последняя стабильная версия `maven-dependency-plugin`^].
<2> Плагин выполняет работу в момент выполнения фазы `package`.
<3> Целевая папка куда должны быть скопированы все зависимости. +
_(В приведенном примере это папка `target/lib`.)_
<4> Данная настройка указывает какой тип зависимостей должен быть скопирован. +
_(Данная опция позволяет исключать библиотеки необходимые для тестирования или библиотеки, которые уже присутствуют на клиентском компьютере.)_

После добавления плагина необходимо выполнить команду:

[source,text]
----
mvn clean package
----

В результате выполнения команды `maven-dependency-plugin` скопирует все зависимости в папку `target/lib`:

.The required dependencies for the simplest REST-based micro service.
image::quick-start/maven-dependency-plugin-results.jpg[]

Теперь команду запуска можно упростить +
_(Вместо перечисления всех библиотек указать папку `lib`)_:

[source,text]
----
java --add-opens=java.base/jdk.internal.misc=io.netty.common -p ./classes:lib -m examples.quick.start/io.rxmicro.examples.quick.start.HelloWorldMicroService
----

include::__note_run_on_windows.adoc[]
