...
Поэтому *RxMicro*, который нуждается в использовании https://openjdk.java.net/projects/jdk/11/[JDK 11^] или выше, требует наличия дескриптора `module-info.java` для любого Вашего микросервисного проекта.
...
_Благодаря транзитивным зависимостям фреймворка RxMicro количество необходимых модулей сильно сокращено_. +
...
Для упрощения коммуникации между REST-based микросервисами, RxMicro фреймворк поддерживает только *JSON объект* в качестве возвращаемого типа любого REST-based микросервиса.
Таким образом любой REST-based микросервис, построенный с помощью фреймворка RxMicro может возвращать *только JSON объекты*.
...
Модификаторы `protected` и `<default>` также поддерживаются фреймворком RxMicro.
====
