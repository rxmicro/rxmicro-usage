...
*RxMicro* предоставляет модули для эффективного написания <<{testing}#tesing-section,любых видов тестов>>.
...
<2> `rxmicro-http-client-jdk` - библиотека http клиента на базе JDK модуля https://developer.oracle.com/java/jdk-http-client.html[`java.net.http`^];
<3> `rxmicro-rest-client-exchange-json` - библиотека конвертации Java модели в https://www.json.org/[JSON^] формат и наоборот на стороне HTTP клиента;
...
<1> Старт процессора аннотаций `io.rxmicro.annotation.processor.RxMicroAnnotationProcessor`.
<2> Успешно отработал процессор аннотаций `io.rxmicro.annotation.processor.RxMicroAnnotationProcessor`.
<3> Старт процессора аннотаций `io.rxmicro.annotation.processor.RxMicroTestsAnnotationProcessor`.
<4> Успешно отработал процессор аннотаций `io.rxmicro.annotation.processor.RxMicroTestsAnnotationProcessor`.
