...
----
<1> Если `@RestServerGeneratorConfig.generateRequestValidators == GenerateOption.DISABLED`, то валидаторы для всех моделей HTTP запросов в текущем проекте не будут сгенерированы.
<2> Если `@RestServerGeneratorConfig.generateResponseValidators == GenerateOption.DISABLED`, то валидаторы для всех моделей HTTP ответов в текущем проекте не будут сгенерированы.
<3> Если `@RestClientGeneratorConfig.generateRequestValidators == GenerateOption.DISABLED`, то валидаторы для всех моделей HTTP запросов в текущем проекте не будут сгенерированы.
<4> Если `@RestClientGeneratorConfig.generateResponseValidators == GenerateOption.DISABLED`, то валидаторы для всех моделей HTTP ответов в текущем проекте не будут сгенерированы.
