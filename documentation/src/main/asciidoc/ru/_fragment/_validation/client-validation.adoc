...
----
<1> Если REST клиент будет получать валидный HTTP ответ, то никаких ошибок не возникнет.
<2> Если поле `email` будет некорректным в HTTP ответе, то REST клиент вернет сигнал ошибки.
...
[NOTE]
====
Для ознакомления с возможностями фреймворка RxMicro для валидации HTTP запросов в REST клиентах, рекомендуем обратится к примеру:
