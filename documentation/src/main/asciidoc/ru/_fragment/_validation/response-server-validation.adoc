...
Что информировать `RxMicro Annotation Processor` о необходимости генерации валидаторов HTTP ответа, необходимо использовать аннотацию <<{rest-controller}#rest-controller-rest-server-generator-config-section,`@RestServerGeneratorConfig`>>:
...
Так как данный модуль добавлен, то `RxMicro Annotation Processor` сгенерирует валидаторы для HTTP ответа.)_
