...
RxMicro предоставляет модуль `rxmicro.logger` для логирования важных событий в процессе работы микросервисов.
...
При активации `java.logging` логгера, RxMicro автоматически преобразует уровни из множества `{OFF, ERROR, WARN, INFO, DEBUG, TRACE, ALL}` в `{OFF, SEVERE, CONFIG, INFO, FINE, FINEST, ALL}`.
