...
*RxMicro* знает об этом механизме и поэтому при генерации конвертера, использует его:
...
Поэтому при компиляции данного класса `RxMicroAnnotationProcessor` выдает предупреждение:
...
[source,java]
----
include::{RX-MICRO-USAGE-ROOT-LOCAL-PATH}benchmarks/src/main/java/io/rxmicro/bencmark/fields/ReadWriteFieldBenchmark.java[tag=content,indent=0]
----
...
[source,txt]
----
include::{RX-MICRO-USAGE-ROOT-LOCAL-PATH}benchmarks/results/ReadWriteFieldBenchmark.txt[]
----
...
[TIP]
====
Таким образом RxMicro использует следующий алгоритм считывание (записи) с полей java модели:
...
_(В процессе генерации такого типа конвертера `RxMicroAnnotationProcessor` информирует разработчика о `PERFORMANCE WARNING`.)_
====

Для использования преимуществ инкапсуляции при проектировании микросервисов с помощью фреймворка RxMicro следуйте следующим рекомендациям:
