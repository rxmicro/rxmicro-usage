...
RxMicro поддерживает маршрутизацию запросов на основании HTTP метода, URL Path и HTTP тела:

<<<

==== Маршрутизации запросов на основании HTTP метода

RxMicro позволяет обрабатывать одинаковые HTTP запросы, отличающиеся *только* HTTP методом разными обработчиками:
...
RxMicro позволяет обрабатывать HTTP запросы на разные URL Paths разными обработчиками:
...
----
<1> Кроме статических URL Paths, RxMicro также поддерживает и <<rest-controller-path-variables-basic,`path variables`.>>
...
Поэтому RxMicro кроме стандартных типов маршрутизации также поддерживает маршрутизацию на основании наличия HTTP тела:
