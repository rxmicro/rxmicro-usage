...
Однако данный механизм не поддерживается фреймворком RxMicro, поэтому типы
{REACTOR-CORE-DOC-ROOT-URL}reactor/core/publisher/Flux.html[`Flux`^] и
{RX-JAVA-DOC-ROOT-URL}io/reactivex/Flowable.html[`Flowable`^]
...
_(Данный тип рекомендован для простейших `stateless` микросервисов без параметров.)_
. Список примитивных параметров. +
_(Данный тип рекомендован для микросервисов, поведение которых зависит от 1-2 параметра.)_
. Пользовательский класс, моделирующий HTTP запрос. +
_(Данный тип рекомендован для микросервисов, поведение которых зависит от 3 и больше параметров.)_
