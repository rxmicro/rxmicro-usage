...
RxMicro поддерживает HTTP заголовки в моделях запроса и ответа:
...
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-model/src/main/java/io/rxmicro/model/MappingStrategy.java#L58[`MappingStrategy.CAPITALIZE_WITH_HYPHEN`^].
...
Для определения имени HTTP заголовка для указанного поля модели RxMicro использует следующий алгоритм:

. Если поле аннотировано аннотацией `@Header` с явным указанием имени HTTP заголовка используется указанное имя;
. Если имя HTTP заголовка не указано в аннотации `@Header`, RxMicro проверяет наличие аннотации `@HeaderMappingStrategy` над классом модели;
...
Для моделей запроса она не имеет смысла, так как фреймворк RxMicro конвертирует в список значений любой из поддерживаемых форматов.
====
