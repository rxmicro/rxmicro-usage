...
<2> После возврата HTTP ответа с перенаправлением запроса HTTP клиент автоматически повторит запрос на новый URL Path.
...
----
<1> По умолчанию встроенный HTTP клиент, который используется для выполнения тестовых запросов к микросервису не поддерживает перенаправление.
