...
* Maven плагин `asciidoctor-maven-plugin`, который конвертирует формат http://asciidoc.org/[`AsciiDoc`^] в https://html.spec.whatwg.org/[`HTML`^] или https://www.adobe.com/content/dam/acom/en/devnet/pdf/pdfs/PDF32000_2008.pdf[`PDF`^], по-умолчанию считывает файлы с папки `$ROOT/src/main/asciidoc`.
* Построение проектной документации может быть использовано в качестве начальной версии документа, а затем генерация может быть отключена и все изменения в документацию будут вносится вручную и сохраняться в системе контроля версий.
...
Плагин `asciidoctor-maven-plugin` позволяет конвертировать документацию в формате http://asciidoc.org/[`AsciiDoc`^] в
...
[IMPORTANT]
====
Обратите внимание на то, что так как мы исключаем артефакты относящиеся к постороению проектной документации из библиотек, необходимых для успешного запуска микросервисного проекта, обязательно нужно использовать модификатор `static` в дескрипторе `module-info.java`: `requires static rxmicro.documentation.asciidoctor;`.
