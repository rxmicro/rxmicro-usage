...
разработчик может перегрупировывать стандартные секции проектной документации, а также подключать свои собственные фрагменты в качестве разделов.
...
[NOTE]
====
Кроме генерации финального http://asciidoc.org/[`AsciiDoc`] документа, `RxMicro Annotation Processor` позволяет генерировать отдельные динамические фрагменты.
