...
Для описания HTTP ответов, которые отличны от успешного, модуль `rxmicro.documentation` предоставляет две аннотации:
...
<4> Параметр `exampleErrorMessage` указывает на значение, которое используется в качестве примера в генерируемой проектной документации.
<5> Аннотация {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-documentation/src/main/java/io/rxmicro/documentation/ModelExceptionErrorResponse.java[`@ModelExceptionErrorResponse`^] позволяет указать класс исключения стандартного
<6> или пользовательского типов.

При использовании пользовательского типа необходимые параметры для построения проектной документации указываются на данном типе:
...
<3> Аннотация {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-documentation/src/main/java/io/rxmicro/documentation/Example.java[`@Example`^] описывает значение, которое используется в качестве примера в генерируемой проектной документации.
