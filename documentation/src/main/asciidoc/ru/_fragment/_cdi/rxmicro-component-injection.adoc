...
Кроме пользовательских классов фреймворк RxMicro поддерживает возможность внедрения of RxMicro components.

Например, если в проекте объявлен декларативный REST клиент:
...
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-rest-client/src/main/java/io/rxmicro/rest/client/RestClientFactory.java#L35[`RestClientFactory.getRestClient`^],
разработчик может использовать механиз внедрения зависимостей:
...
<2> Кроме внедрения REST клиентов, фреймворк RxMicro поддерживает также внедрение конфигураций. +
