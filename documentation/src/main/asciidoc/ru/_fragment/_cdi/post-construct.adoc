...
_(За исключением механизма внедрения с помощью конструктора, конечно же!)_ Для того, чтобы выполнить какой-либо код в процессе создания объекта, но уже после внедрения всех зависимостей в данный объект, фреймворк RxMicro предоставляет специальную аннотацию
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-cdi/src/main/java/io/rxmicro/cdi/PostConstruct.java[`@PostConstruct`^].

Таким образом если в классе сущесвует метод, аннотированный аннотацией {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-cdi/src/main/java/io/rxmicro/cdi/PostConstruct.java[`@PostConstruct`^], то этот метод автоматически вызывается после внедрения всех зависимостей в созданный объект данного класса.
