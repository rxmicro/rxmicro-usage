...
Если в текущем модуле проекта присутсвует два и больше реализаций одного и того же интерфейса, то в процессе внедрения может возникнуть проблема разрешения неоднозначностей.
Данная проблема возникает из-за того, что в поле с типом интерфейса потенциально может быть внедрена реализация любого из классов наследников данного интерфейса.
...
Т.е. алгоритм разрешения неоднозначностей по-умолчанию активируется *только при наличии таких однозначностей!*
====
...
_(Это неявное имя, которое задается фреймворком RxMicro для класса `ProductionBusinessService`!)_
...
_(Это неявное имя, которое задается фреймворком RxMicro для класса `DevelopmentBusinessService`!)_
...
При использовании явных имен для классов реализаций, в качестве квалификатора точки внедрения необходимо указывать именно эти явные имена:
...
При использовании строковых имен для классов реализаций и точек внедрения разработчик может допустить ошибку.
...
----
<1> Для того, чтобы пользовательская аннотация определялась фреймворком RxMicro как квалификатор, необходимо ее аннотировать аннотацией
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-cdi/src/main/java/io/rxmicro/cdi/Named.java[`@Named`^] или
...
<2> Для контроля над именами классов и точек внедрения компилятором, рекомендуется использовать перечисления.
