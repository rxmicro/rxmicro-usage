...
Фреймворк RxMicro ориентирован на создание микросервисных проектов.
Одним из ключевых особенностей микросервисов является их простота.
Именно поэтому в качестве основной и единственной scope был выбран `singleton` scope.

Таким образом *все CDI компоненты являются `singletons`!*

Это означает что в момент запуска микросервисного проекта создается *только один объект* класса реализации компонента и внедряется во все необходимые точки внедрения.

[NOTE]
====
Если необходимо в каждую точку внедрения внедрять отдельный объект класса реализации (`prototype` scope), то в этом случае необходимо использовать <<cdi-factory-class-section, фабрику, создающую объекты заданного класса>>!
====
