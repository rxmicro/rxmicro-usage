...
По-умолчанию все точки внедрения являются обязательными.
Это означает, что если в процессе внедрения зависимостей фреймворк RxMicro не обнаружит подходящего объекта, то возникнет ошибка.
...
В случае активации режима `optional injection` фреймворк RxMicro использует следующий алгоритм для внедрения:
