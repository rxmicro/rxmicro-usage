...
Если фреймворк RxMicro обнаруживает метод в классе, аннотированный аннотацией {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-cdi/src/main/java/io/rxmicro/cdi/Factory.java[`@Factory`^],
то при создании объект данного класса использует именно этот метод, а не констуктор:
