...
Фреймворк RxMicro поддерживает *Multibindings*.

*Multibindings* - это функция CDI контейнера, которая позволяет найти все реализации данного типа и внедрить их.

Например, если сущесвует два класса реализации интерфейса `BusinessService`:
...
то данные классы могут быть внедрены фреймворком RxMicro в поле типа {JDK-JAVA-BASE-DOC-ROOT-URL}java/util/Set.html[`java.util.Set`^]:
