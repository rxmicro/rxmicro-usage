...
Для активации модуля `rxmicro.cdi`, необходимо выполнить два шага:

* Подключить зависимость `rxmicro-cdi` в `pom.xml` микросервисного проекта:
...
* Добавить модуль `rxmicro.cdi` в дескриптор `module-info.java` текущего проекта:
...
После активации модуля `rxmicro.cdi` можно создавать бизнес сервис:
...
ничем принципиальным не отличается от
...
Корректность внедрения зависимости может быть проверена с помощью <<testing#testing-rest-based-microservice-tests-section, теста REST-based микросервиса>>:
