...
https://openjdk.java.net/projects/jdk9/[On September 21, 2017], Java 9 has introduced https://www.oracle.com/corporate/features/understanding-java-9-modules.html[JPMS^].
...
Currently, the majority of existing frameworks and libraries are focused on https://openjdk.java.net/projects/jdk8/[JDK 8] and do not support JPMS.
...
For integration with libraries that do not support JPMS, the RxMicro framework allows You to enable the `unnamed module` mode for the `RxMicro Annotation Processor`.
