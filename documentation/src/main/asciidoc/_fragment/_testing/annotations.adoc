...
_RxMicro поддерживает компонент `BlockingHttpClient` только для <<{testing}#testing-rest-based-microservice-tests-section,тестов REST-based микросервиса>> и <<{testing}#testing-integration-tests-section,интеграционных тестов REST-based микросервисов>>._
