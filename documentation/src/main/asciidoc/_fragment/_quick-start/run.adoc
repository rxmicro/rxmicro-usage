...
folder, where `$HOME` is the user's home directory:

* For Linux systems: `$HOME` = `/home/$USER`.
* For Osx systems: `$HOME` = `/Users/$USER`.
* For Windows systems: `$HOME` = `C:\Documents and Settings\%USERNAME%`.
====
