...
[NOTE]
====
To learn more about how the annotation processor works in the test environment, please go to the following section: <<{testing}#testing-how-it-works-section>>.
====
...
This interface allows to execute *blocking* requests to the microservice via the `HTTP` protocol.
This field is initialized automatically when running the test with `reflection`. Upon initialization it refers to the test HTTP server that was automatically started for the test.
...
[NOTE]
====
For low-level and effective work with JSON format, RxMicro provides a separate `rxmicro.json` module.

To get a general idea of the capabilities of this module, which are required when writing tests, go to the following section: <<{core}#core-rx-micro-json-section>>.
====
...
[NOTE]
====
For further information on how the annotation processor works for the test environment, go to the following section: <<{testing}#testing-how-it-works-section>>.
====
