...
[IMPORTANT]
====
By default, the https://github.com/r2dbc/r2dbc-postgresql[reactive R2DBC PostgreSQL driver^] uses the {REACTOR-CORE-DOC-USER_GUIDE}[*Project Reactor*^] library, so when adding the `rxmicro.data.sql.r2dbc.postgresql` module, the `reactor.core` module is automatically added and available for usage!
====
