...
To write <<{testing}#testing-section,tests>> correctly, it is necessary to have a general idea about the `rxmicro.json` module.
...
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-http-client/src/main/java/io/rxmicro/http/client/ClientHttpResponse.java#L38[`ClientHttpResponse.body()`^]method)_:
...
----
<1>  The `ClientHttpResponse.body()` method returns the HTTP response body.
...
*RxMicro always arranges JSON object properties!*
...
*Thus, RxMicro recommends using the `rxmicro.json` module to compare JSON objects, but at the same time provides an opportunity to use any other JSON framework!*
====
