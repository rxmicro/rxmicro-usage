...
[TIP]
====
The {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-json/src/main/java/io/rxmicro/json/JsonNumber.java[`io.rxmicro.json.JsonNumber`^]
class extends the {JDK-JAVA-BASE-DOC-ROOT-URL}java/lang/Number.html[`java.lang.Number`^] class.

Therefore, *to work with numeric data types, You can use the `java.lang.Number` type instead of `io.rxmicro.json.JsonNumber`*!
====
