...
appender, which outputs all error logging information into `System.err` and all other levels into `System.out`.
...
* To enable `System.err` output for all log levels use the following configuration:
[source,properties]
----
io.rxmicro.logger.jul.SystemConsoleHandler.stream=stderror
----

* To enable `System.out` output for all log levels use the following configuration:
[source,properties]
----
...
{RX-MICRO-JAVADOC-ROOT-URL}rxmicro-logger/latest/rxmicro.logger/io/rxmicro/logger/jul/SystemConsoleHandler.html[^]
====
