...
To override the default value, the RxMicro provides the
...
_(A microservice project may contain several classes of REST controllers, so the general settings are configured using the `module-info.java` descriptor rather than the REST controller class.)_
...
While reading the configuration of the `HttpServerConfig` class, the RxMicro reads the default values set with annotations:
...
In other words, when using configuration via annotations, the RxMicro can also read other configuration sources.
...
[NOTE]
====
For the test environment only, RxMicro provides special <<{testing}#testing-annotations-WithConfig-inline,`@WithConfig`>> annotation.
...
[.example-code-link]
{RX-MICRO-EXAMPLES-SRC-ROOT-TREE-URL}config-annotations-supplier[`https://github.com/rxmicro/rxmicro-usage/tree/master/examples/config-annotations-supplier`^]
====
