...
_(Для получения подробной информации о преимуществах модификатора `static` обратитесь к <<project-documentation-basic-usage-maven-dependency-plugin>>.)_

После выполнения данных шагов в процессе компиляции микросервисного проекта, the `RxMicro Annotation Processor` сгенерирует файл `ExamplesDocumentationAsciidoctorQuickStartDocumentation.adoc`.
...
_(Для настройки папки, в которую должна быть сгенерирована REST-based microservice documentation, обратитесь к <<core#rxmicro-annotation-processor-options-section>>.)_
...
Статические фрагменты (описание бизнес задачи, диаграммы и рисунки) будут создаваться разработчиком вручную.
Динамические фрагменты будут генерироваться by the `RxMicro Annotation Processor` при каждой компиляции проекта.
