...
<2> Besides supporting HTTP request model classes, RxMicro supports request handlers that accept HTTP request parameters as method parameters.
...
<4> To declare a method parameter as an HTTP header field, You do not need to use any additional annotations.
...
[IMPORTANT]
====
*Note that RxMicro does not distinguish whether HTTP request parameters will be transferred to the handler using the start line or HTTP body!*
...
Therefore, besides primitives RxMicro also supports nested JSON objects and arrays.
