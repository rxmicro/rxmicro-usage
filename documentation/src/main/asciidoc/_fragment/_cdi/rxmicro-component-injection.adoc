...
then instead of getting an explicit link to the implementation class instance using the
{RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-rest-client/src/main/java/io/rxmicro/rest/client/RestClientFactory.java#L35[`RestClientFactory.getRestClient`^], the developer can use the dependency injection mechanism:
...
----
<1> To get a link to the REST client implementation class, the developer can use the {RX-MICRO-SRC-ROOT-BLOB-URL}rxmicro-cdi/src/main/java/io/rxmicro/cdi/Inject.java[`@Inject`^] annotation.
